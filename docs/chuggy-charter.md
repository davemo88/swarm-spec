...
| Visibility invariant | **Adopted, and split 2026-08-14 into the half a state predicate can carry and the half it cannot.** Checkable, and now checked: **no structural deadlock** — every live ticket can still reach Done, or was settled by its author, or holds an open desk task (`noStructuralDeadlock`). Not checkable by any state predicate: whether a ticket that *can* progress ever *does* — an abandoned Draft, an unpicked dispatch, an unwon dequeue all leave a reachable route to Done, so no invariant distinguishes them from healthy waiting; those are accepted by the row above. The old `deskVisibility` predicate is renamed `stuckSubsetCovered` because it was a tautology over its own definitions and could not fail for any machine — which is how the grouping deadlock passed it. (Definition of "progressing" remains contested — §4.) | both, `humans/invariant`; split by davemo88, 2026-08-14 |
...
| Termination, committed part | **Per-ticket work-boundedness is owed**: given the fabric axioms, every ticket performs finitely many acting steps — no ticket churns forever. **Waiting is explicitly NOT bounded**: whether a ticket is ever chosen to act belongs to the author (release), the dispatcher (the agentic pick) and the gate (the dequeue). | both rounds, both respondents on the original liveness row; **narrowed 2026-08-14 by davemo88** — see the row below |
| Bounds work, not waiting | **Accepted, 2026-08-14 (davemo88).** The original row read "per-job liveness is owed: every job reaches Done, Escalated, or Stalled, or provably parks", which is not provable against the standing "no queue, no fairness" non-goal: closing it needs weak fairness in its **per-ticket** form ("a ticket that stays continuously Ready is eventually dispatched"), and the action-level reading does not suffice either — `dispatch`'s pick ranges over the whole Ready set, so a dispatcher that forever prefers other work takes the action infinitely often and still starves one ticket. The contradiction is resolved **in favour of the non-goal**: *"if there is always more pressing work, that's a real possibility."* Every account bounds what a ticket can DO; nothing bounds how long it WAITS. This generalizes proposed R6's accepted-unbounded queue wait from the landing queue to the whole model. | davemo88, 2026-08-14; supersedes the unamended termination row |
