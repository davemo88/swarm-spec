...
1. **The adopted reading admitted a reachable deadlock.** `absorbableIn`
   ...
   — now `stuckSubsetCovered`, and now known to be a tautology that could
   not have caught this or anything else — held **vacuously** over a
   permanent deadlock no human task owned; `noStructuralDeadlock` is the
   invariant added afterwards that does catch it).
...
| ~~R2's group half / R7~~ | **Deleted** (grouping removal): `PBatched` + its rung, `BatchMark`/`Ticket.batch`, `membersOf`/`absorbableIn`/`groupDeps`/`decideAbsorb`, the `absorb` action, `landSuccess`'s fan-out (back to the single transition), the dissolution/park arms in `decideRevoke`, `batchWellFormed`, `landingExclusive`'s mark conjunct, the group edges in the visibility walk (the fixpoint shape kept, the edges gone), the BATCHING set + its `stepDescends` arm, the batch witness module, and this suite's group tests. The severability claim was tested by exercising it. |
...
- **No grouping** (R7 and R2's group half withdrawn): tickets are not
  batched in any form. The merge-queue PR's absorbed-lead reading admitted
  a reachable dep cycle through the derived union and modelled none of
  chuggernaut's actual batch; a batch is expressible as an ordinary ticket
  whose brief covers the work. See the removal section above.
...
- **The dispatcher is agentic and modeled as such** (notes PR): dispatch
  ...
  (non-goals unchanged).
- **No structural deadlock** (charter §2's visibility row, checkable
  half): every live ticket can still reach Done, or was settled by its
  author, or holds an open desk task (`noStructuralDeadlock` —
  `canFinishSet` is a least fixpoint UPWARD from the terminal over
  `gateDeps`, deliberately the opposite direction from `stuckSet`, because
  a dependency cycle has no base case for an outward walk to start from).
  Validated in both directions: green on this tree (unit, all eight
  witness modules, four mc instances × 2000 traces) and **red on the
  pre-removal machine**, where it fails the grouping finding's own
  reproducer at exactly the absorb step and is found by random search in
  478ms — teeth demonstrated against a real defect, not argued.
  `stuckSubsetCovered` (ex-`deskVisibility`) is retained as a guard on the
  two walks against each other; it is a tautology over its definitions and
  its doc comment now says so.
- **Per-ticket work-boundedness — chuggy bounds work, not waiting** (the
  PR 1 gate, extended by PR 2; retitled 2026-08-14 from "per-ticket
  liveness, sketched", which claimed a conclusion descent never reached —
  closing it needs per-ticket weak fairness on `dispatch`, a standing
  non-goal, and the gap is now accepted by name in charter §2): every step
  outside the named
  STUTTER/CHURN/AUTHORING sets strictly decreases a nonnegative
  measure
