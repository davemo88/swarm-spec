...
| **R7a** | **A group must be CONVEX in the dep graph.** No ticket outside the group may sit on a live dep path back into it — enforced at the guard (`absorbKeepsConvex`, a conjunct of `absorbableIn`) and stated as a checked theorem (`groupDepsAcyclic`). Without it, absorption builds a **deadlock**: with X depping the lead and m depping X, absorbing m puts X in the derived union while X still waits on the lead, and group → X → group waits forever. **Derived, not proposed** — the union's drop-out is R7's own mechanic, and this is the precondition under which it is sound (necessary as well as sufficient: any non-convex absorption puts the separating ticket in the union by construction). | Found against the merge-queue PR's own machine, not against v1 — the direct-dep conjunct at `absorbableIn` refuses one hop, and the drop-out reaches arbitrarily far. Invisible to every prior check: `depsAcyclic` reads the STORED deps, which absorption never rewrites, and `stuckSet`'s only base case is `PEscalated`, which a pure cycle never reaches — so the deadlocked tickets are in neither `stuckSet` nor `coveredSet` and `deskVisibility` holds **vacuously** over them, which is the §5c wedge hazard the charter §2 visibility row exists to answer. Witnessed deterministically (`tests/chuggy_batch_cycle_test.qnt`, the refusal pinned mid-trace with the pre-convexity guard expression carried verbatim beside it) and randomly (`mc/mc_batch_cycle.qnt`). |
...
| R7a (group convexity) | `depsReachLive`/`absorbKeepsConvex` (the `absorbableIn` conjunct), the `groupDepsAcyclic` invariant, `tests/chuggy_batch_cycle_test.qnt`, `mc/mc_batch_cycle.qnt`, their two runner lines. Severable from R7 only in the sense that dropping it re-opens the deadlock — it has no other dependents. |
